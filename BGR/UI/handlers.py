...
def handle_upload_click(canvas, upload_text_id):
    ...
    canvas.itemconfig(
        upload_text_id, text=f"{len(state.selected_files_paths)} files loaded")
...
def handle_remove_click(root, canvas):
    ...
    status_tag = canvas.create_text(
        350, 320, text="Starting...", font=state.DEFAULT_FONT)
    root.update_idletasks()

    for index, file_path in enumerate(state.selected_files_paths, 1):
        input_path = Path(file_path)
        save_folder = Path(
            state.output_directory) if state.output_directory else input_path.parent
        output_path = save_folder / \
            f"{input_path.stem}_no_bg.{state.selected_export_format}"

        canvas.itemconfig(
            status_tag, text=f"Processing {index}/{len(state.selected_files_paths)}...")
        root.update()

        try:
            remove_background_from_image(str(input_path), str(
                output_path), state.selected_export_format)
        except Exception as e:
            print(f"Error: {e}")

    canvas.itemconfig(status_tag, text="Batch Complete!", fill="green")
    root.after(2000, lambda: canvas.itemconfig(status_tag, text=""))


def show_format_menu(event, root, canvas, format_text_id):
    format_menu = tk.Menu(root, tearoff=0, bg="#FFF0CF",
                          activebackground="#F7D88A", font=("Arial", 10))
    supported_formats = [("PNG", "png"), ("WebP", "webp"),
                         ("BMP", "bmp"), ("TIFF", "tiff"), ("JPEG", "jpg")]

    for label, ext in supported_formats:
        format_menu.add_command(
            label=label,
            command=lambda e=ext: set_selected_export_format(
                e, canvas, format_text_id)
        )
